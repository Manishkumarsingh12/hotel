...
$hbmsaid=$_SESSION['hbmsaid'];
 ...
 if(!empty($room_no) && (checkRoomNoAbility($room_no,$eid)=='1')){
	
	echo '<script>alert("This Room no is Already Exist")</script>'; 
	echo '<script>history.back()</script>'; die;
}

$sql="update tblroom set RoomType=:roomtype,RoomNo=:RoomNo,RoomName=:roomname,MaxAdult=:maxadult,MaxChild=:maxchild,RoomDesc=:roomdes,NoofBed=:nobed,RoomFacility=:roomfac where ID=:eid";

$query=$dbh->prepare($sql);

$query->bindParam(':roomtype',$roomtype,PDO::PARAM_STR);

$query->bindParam(':RoomNo',$room_no,PDO::PARAM_STR);
...
<option value="<?php echo htmlentities($row2->ID);?>"><?php echo htmlentities($row2->CategoryName);?></option>
 ...
 <?php } ?> 
													...
													<div class="form-group"> <label for="exampleInputEmail1">Room Number</label> <input type="text" class="form-control" name="room_no" value="<?php  echo $row->RoomNo;?>" required='true'>
	...
	$(document).ready(function() {});
        ...
        var flag=false;												
				$("input[name=room_no]").on('keyup',function(){
					...
					console.log(res);
					if(res!=''){
						data_string='room_no=' + res+'&RoomID='+ID+'&updateCheckRoomBookNo='+1;
						$.ajax({
